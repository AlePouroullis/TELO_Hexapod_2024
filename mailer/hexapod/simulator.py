...
class Simulator:
	...
	def __init__(self, controller, urdf='/urdf/hexapod_simplified.urdf', visualiser=False, follow=True, collision_fatal=True, failed_legs=[], camera_position=[0, 0, 0], camera_distance=0.7, camera_yaw=20, camera_pitch=-30):
		...
		self.dt = 1.0/240.0 #240.0  #: float: Timestep of simulator. Default is 1/240s for PyBullet.
		self.n_step = 0
		...
		for failed_leg in failed_legs:
			self.locked_joints.extend([failed_leg*3-3, failed_leg*3-2, failed_leg*3-1])
	...
	def get_supporting_legs(self):
		"""Determines the supporting legs for the hexapod

		Note
		----
		A leg is considered to be supporting if it is in contact with the ground.
...
def evaluate_gait(leg_params, body_height=0.14, velocity=0.3, duration=5.0, visualiser=True, collisions=False, failed_legs=[]):
